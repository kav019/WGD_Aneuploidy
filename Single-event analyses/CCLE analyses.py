...
def data_exploration(ccle_mat):
    ...
    for cell_line in ccle_mat.CCLE_ID:
        ...
        if not ccle_mat[
            (ccle_mat.CCLE_ID == cell_line) & (ccle_mat.ploidy < 2.5)
        ].empty:
            ...
            (ccle_mat.CCLE_ID == cell_line) & (ccle_mat.ploidy > 3)
        ].empty:
...
def analyze_whole_chr_anp_frac_samplewise(types):
    ...
    for type in types:
        ...
        wgd_plus_df_non_acro = wgd_plus_df.drop(
            ["13q", "14q", "15q", "21q", "22q"], axis=1
        )
        wgd_minus_df_non_acro = wgd_minus_df.drop(
            ["13q", "14q", "15q", "21q", "22q"], axis=1
        )
        ...
        for i in range(0, len(wgd_plus_df_non_acro.index)):
            ...
            for j in range(0, 34, 2):
                ...
                total_wgd_plus += np.sum(
                    np.logical_and(
                        wgd_plus_df_non_acro.iloc[i, j] == 1,
                        wgd_plus_df_non_acro.iloc[i, j + 1] == 1,
                    )
                ) + np.sum(
                    np.logical_and(
                        wgd_plus_df_non_acro.iloc[i, j] == -1,
                ...
                if not pd.isnull(wgd_plus_df_non_acro.iloc[i, j]) and not pd.isnull(
                    wgd_plus_df_non_acro.iloc[i, j + 1]
                ):

                    if (
                        wgd_plus_df_non_acro.iloc[i, j] != 0
                        and wgd_plus_df_non_acro.iloc[i, j + 1] == 0
                    ):
                        arm_level += 1
                        ...
                        wgd_plus_df_non_acro.iloc[i, j] != 0
                        and wgd_plus_df_non_acro.iloc[i, j + 1] != 0
                        and wgd_plus_df_non_acro.iloc[i, j]
                        != wgd_plus_df_non_acro.iloc[i, j + 1]
                    ):  # one gain, one loss
                        arm_level += 2
                    ...
                    if (
                        not pd.isnull(wgd_plus_df_non_acro.iloc[i, j])
                        and wgd_plus_df_non_acro.iloc[i, j] != 0
                    ):
                        arm_level += 1

                    elif (
                        not pd.isnull(wgd_plus_df_non_acro.iloc[i, j + 1])
                        and wgd_plus_df_non_acro.iloc[i, j + 1] != 0
                    ):
                        arm_level += 1
            ...
            value_series = [
                [
                    ...
                    arm_level,
                    total_wgd_plus,
                    total_wgd_plus / (total_wgd_plus + arm_level),
                ]
            ]
            ...
            whole_chr_anp_frac_wgd_plus.append(
                total_wgd_plus / (total_wgd_plus + arm_level)
            )
        ...
        for i in range(0, len(wgd_minus_df_non_acro.index)):
            ...
            arm_level = 0
            for j in range(0, 34, 2):
                total_wgd_minus += np.sum(
                    np.logical_and(
                        wgd_minus_df_non_acro.iloc[i, j] == 1,
                        wgd_minus_df_non_acro.iloc[i, j + 1] == 1,
                    )
                ) + np.sum(
                    np.logical_and(
                        wgd_minus_df_non_acro.iloc[i, j] == -1,
                ...
                if not pd.isnull(wgd_minus_df_non_acro.iloc[i, j]) and not pd.isnull(
                    wgd_minus_df_non_acro.iloc[i, j + 1]
                ):

                    if (
                        wgd_minus_df_non_acro.iloc[i, j] != 0
                        and wgd_minus_df_non_acro.iloc[i, j + 1] == 0
                    ):
                        arm_level += 1
                        ...
                        wgd_minus_df_non_acro.iloc[i, j] != 0
                        and wgd_minus_df_non_acro.iloc[i, j + 1] != 0
                        and wgd_minus_df_non_acro.iloc[i, j]
                        != wgd_minus_df_non_acro.iloc[i, j + 1]
                    ):  # one gain, one loss
                        arm_level += 2
                    ...
                    if (
                        not pd.isnull(wgd_minus_df_non_acro.iloc[i, j])
                        and wgd_minus_df_non_acro.iloc[i, j] != 0
                    ):
                        arm_level += 1

                    elif (
                        not pd.isnull(wgd_minus_df_non_acro.iloc[i, j + 1])
                        and wgd_minus_df_non_acro.iloc[i, j + 1] != 0
                    ):
                        arm_level += 1
            ...
            value_series = [
                [
                    ...
                    arm_level,
                    total_wgd_minus,
                    total_wgd_minus / (total_wgd_minus + arm_level),
                ]
            ]
            ...
            whole_chr_anp_frac_wgd_minus.append(
                total_wgd_minus / (total_wgd_minus + arm_level)
            )

        whole_chr_anp_frac_sample = (
            whole_chr_anp_frac_wgd_plus + whole_chr_anp_frac_wgd_minus
        )

        whole_chr_anp_frac_sample = pd.Series(whole_chr_anp_frac_sample)
        wgd_status = ["WGD+"] * len(wgd_plus_df_non_acro.index) + ["WGD-"] * len(
            wgd_minus_df_non_acro.index
        )
        wgd_status = pd.Series(wgd_status)
        type_vec = pd.Series(
            [type]
            * (len(wgd_plus_df_non_acro.index) + len(wgd_minus_df_non_acro.index))
        )

        whole_chr_anp_frac_sample = pd.concat(
            [whole_chr_anp_frac_sample, wgd_status, type_vec], axis=1
        )
    ...
    plot_boxplot(
        pan_cancer_df, "Pan-cancer whole-chr aneuploidy fraction (sample-wise)"
    )
    pan_cancer_df.to_csv(
        "C:\\Users\\kiwii\\Documents\\Kavya\\Uri\\Work\\Tasks\\CCLE analysis\\Prelim\\wca_samplewise_no-ke97.tsv",
        sep="\t",
        index=0,
    )
...
def analyze_whole_chr_anp_frac_samplewise_all(wgd_plus_fracs, wgd_minus_fracs):
    ...
    print(df_all.head())
    df_all.to_csv(
        os.path.join(src_folder, "Type_wise_df_wca_frac.tsv"), sep="\t", index=0
    )
...
def analyze_aneuploidy_score(types):
    ...
    df_scores = pd.concat(
        [pd.Series(all_scores), pd.Series(wgd_status), pd.Series(type_vector)],
        axis=1,
        ignore_index=True,
    )
    ...
    df_scores.to_csv(
        "C:\\Users\\kiwii\\Documents\\Kavya\\Uri\\Work\\Tasks\\CCLE analysis\\Prelim\\anp_scores_samplewise_no-ke97.tsv",
        sep="\t",
        index=0,
    )
...
def compute_wgd_plus_frac(types):
    ...
    pan_cancer_df = pd.DataFrame(
        columns=["Type", "WGD+ count", "WGD- count", "WGD+ fraction"]
    )
    pan_cancer_df = pd.DataFrame()
    type_wise_df = pd.DataFrame()
    ...
    for type in types:
        ...
        pan_cancer_df = pan_cancer_df.append(
            {
                "Type:": type,
                "WGD+ count": len(wgd_plus_df.index),
                "WGD- count": len(wgd_minus_df.index),
                "WGD+ fraction": len(wgd_plus_df.index)
                / (len(wgd_plus_df.index) + len(wgd_minus_df.index)),
            },
            ignore_index=True,
        )
    ...
    pan_cancer_df.to_csv(
        "C:\\Users\\kiwii\\OneDrive\\Documents\\Kavya\\Uri\\Work\\Tasks\\CCLE analysis\\Prelim\\wgd_plus_frac_no-ke97.tsv",
        sep="\t",
        index=0,
    )
...
def analyze_aneuploidy_score_experimental(types):
    ...
    for type in types:
        ...
        for j in range(0, len(wgd_plus_df.index)):
            ...
            while i < 39:
                ...
                # print("i:", i)
                if i != 38:
                    if re.findall(r"\d+", wgd_plus_df.columns[i]) == re.findall(
                        r"\d+", wgd_plus_df.columns[i + 1]
                    ):
                        if (
                            (wgd_plus_df.iloc[j, i] == wgd_plus_df.iloc[j, i + 1])
                            and (wgd_plus_df.iloc[j, i] != 0)
                            and (wgd_plus_df.iloc[j, i] != None)
                            and (wgd_plus_df.iloc[j, i + 1] != None)
                        ):
                            count_anp_sample += 1
                        else:
        ...
        for j in range(0, len(wgd_minus_df.index)):
            ...
            while i < 39:
                ...
                # print("i: ", i)
                if i != 38:
                    if re.findall(r"\d+", wgd_minus_df.columns[i]) == re.findall(
                        r"\d+", wgd_minus_df.columns[i + 1]
                    ):
                        if (
                            (wgd_minus_df.iloc[j, i] == wgd_minus_df.iloc[j, i + 1])
                            and (wgd_minus_df.iloc[j, i] != 0)
                            and (wgd_minus_df.iloc[j, i] != None)
                            and (wgd_minus_df.iloc[j, i + 1] != None)
                        ):
                            count_anp_sample += 1
                        else:
    ...
    df_scores.to_csv(
        "C:\\Users\\kiwii\\OneDrive\\Documents\\Kavya\\Uri\\Work\\Tasks\\CCLE analysis\\Prelim\\anp_scores_samplewise_no-ke97_wca.tsv",
        sep="\t",
        index=0,
    )
...
def analyze_aneuploidy_score_all(types):
    ...
    wgd_status = (["WGD+"] * len(score_wgd_plus)) + (["WGD-"] * len(score_wgd_minus))
    df_scores = pd.concat(
        [pd.Series(all_scores), pd.Series(wgd_status)], axis=1, ignore_index=True
    )
...
def analyze_loss_frac(types):
    ...
    wgd_status = (["WGD+"] * len(fractions_wgd_plus)) + (
        ["WGD-"] * len(fractions_wgd_minus)
    )
    type_vector = types_vector_wgd_plus + types_vector_wgd_minus
    ...
    df_scores.to_csv(
        "C:\\Users\\kiwii\\Documents\\Kavya\\Uri\\Work\\Tasks\\CCLE analysis\\Prelim\\loss_frac.tsv",
        sep="\t",
        index=0,
    )
...
if __name__ == "__main__":
    ...
    input_df = pd.read_csv(
        os.path.join(src_folder, "Type_wise_df_no-ke97.tsv"), sep="\t"
    )
